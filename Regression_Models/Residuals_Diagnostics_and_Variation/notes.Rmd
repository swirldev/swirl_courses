...
* At least the 4 defaults of the 6 plots of plot.lm: 
  ...
  * residuals against leverages (leverage is hat value I think)
...
* `influence.measures` a.k.a, Deletion Diagnostics
  ...
  * `dffits` - the change in the predicted value for a point, obtained when that point is left out of the regression, "Studentized" by dividing by the estimated standard deviation of the fit at that point:
  * `dfbetas` - change in individual coefficients when the $i^{th}$ point is deleted in fitting the model.
  * `cooks.distance` - overall change in the coefficients when the $i^{th}$ point is deleted.
  * `resid` - returns the ordinary residuals
  * `resid(fit) / (1 - hatvalues(fit))` where `fit` is the linear model fit returns the PRESS residuals, i.e. the leave one out cross validation residuals - the difference in the response and the predicted response at data point $i$, where it was not included in the model fitting.
  
All of these admit elementary explanations as shown in the analysis below.
...
### Studentized residuals by elementary mean-shift

Since linear algebra should be avoided the mean-shift derivation of Studentized residuals should be illustrated. Numerical demo below.
...
fit <- lm(Fertility ~ ., swiss) # straightforward fit
swiss[,"dummy"] <- as.numeric(1:47 == 1) # add dummy for first sample
fit.1 <- lm(Fertility ~ ., swiss) # fit using dummy
dummy.coef <- summary(fit.1)$coefficients["dummy",] # extract coefficient of dummy & its std error
c(dummy.coef[1]/dummy.coef[2], rstudent(fit)[1]) # show the normalized coef equals studentized residual.
```
...
### PRESS residuals used to define hat values.

Brian: PRESS residuals are resid(fit) / (1 - hatvalues(fit)) where fit.press is the linear model which returns the PRESS residuals, i.e. the leave one out cross validation residuals - the difference in the response and the predicted response at data point i, where it was not included in the model fitting.

```{r}
fit.m1 <- lm(Fertility ~ . -dummy, swiss[-1,]) # fit omitting first sample
yval.m1 <- predict(fit.m1, swiss[1,]) # predict the first sample based on that fit
res.m1 <- swiss[1,"Fertility"] - yval.m1 # calculate the residual of the prediction
c(res.m1, resid(fit)[1]/(1-hatvalues(fit)[1])) # show Brian's equality is true
```

$$h_i = 1 - \frac{E_i}{E_{i,[-i]}}$$

###  predict(lm(y ~ x, out2[-1,]), out2[1,]) = - coefficient of the dummy variable in mean shift.
