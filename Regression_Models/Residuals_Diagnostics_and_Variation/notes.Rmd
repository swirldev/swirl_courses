...
swiss[,"dummy"] <- as.numeric(1:47 == 1)
fit.1 <- lm(Fertility ~ ., swiss)
dummy.coef <- summary(fit.1)$coefficients["dummy",]
c(dummy.coef[1]/dummy.coef[2], rstudent(fit)[1])
```
...
Brian: resid(fit) / (1 - hatvalues(fit)) where fit is the linear model fit returns the PRESS residuals, i.e. the leave one out cross validation residuals - the difference in the response and the predicted response at data point i, where it was not included in the model fitting.

```{r}
fit.m1 <- lm(Fertility ~ . -dummy, swiss[-1,])
yval.m1 <- predict(fit.m1, swiss[1,])
res.m1 <- swiss[1,"Fertility"] - yval.m1
c(res.m1, resid(fit)[1]/(1-hatvalues(fit)[1]))
```
